...
class AdditivePoissonNoise(IFunctionFactory):

    def __init__(self, parent=None):
        super().__init__(parent)

    def _init_name(self) -> str:
        return "Additive Poisson Noise"
    ...
    def _init_parameters(self) -> List[Parameter]:
        ...
        scale = TupleParameter('Scale sampled per image:', 'scale', (StringDescription('from'), NumberParameter('scale', 'scale', 0, Constraints(0, 255, 0.01, float)), StringDescription('to'), NumberParameter('scale', 'scale', 0, Constraints(0, 255, 0.01, float))))
